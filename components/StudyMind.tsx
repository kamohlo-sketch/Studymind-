...
function LoadingSpinner({ color = COLORS.accent }: { color?: string }) {
  return (
    <div style={{
      width: 20, height: 20,
...
function AITypingEffect({ text, onDone }: { text: string; onDone: () => void }) {
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function Dashboard({ xp, streak, setPage }: { xp: number; streak: number; setPage: (page: string) => void }) {
...
// ─── AI SUMMARIZER ────────────────────────────────────────────────────────────
function Summarizer({ addXP }: { addXP: (amount: number) => void }) {
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const [mode, setMode] = useState<"summary" | "bullets" | "simple" | "guide">("summary");
  const [typed, setTyped] = useState(false);

  const modes: Array<{ id: "summary" | "bullets" | "simple" | "guide"; label: string; icon: string }> = [
...
// ─── FLASHCARDS ───────────────────────────────────────────────────────────────
function Flashcards({ addXP }: { addXP: (amount: number) => void }) {
...
No extra text. Notes:\n${input}`;
  ...
  const diffColors: Record<FlashcardDifficulty, string> = { easy: "#22C55E", medium: COLORS.gold, hard: COLORS.rose };
...
// ─── STUDY TIMER ──────────────────────────────────────────────────────────────
function StudyTimer({ addXP }: { addXP: (amount: number) => void }) {
...
// ─── AI HOMEWORK HELPER ───────────────────────────────────────────────────────
function HomeworkHelper({ addXP }: { addXP: (amount: number) => void }) {
