...
// ─── UTILITY COMPONENTS ────────────────────────────────────────────────────────
function Card({ children, style, className = "", onClick }: any) {
  return (
    <div
      onClick={onClick}
      className={className}
...
function LoadingSpinner({ color = COLORS.accent }: any) {
  return (
    <div style={{
      width: 20, height: 20,
...
function AITypingEffect({ text, onDone }: any) {
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function Dashboard({ xp, streak, setPage }: any) {
...
function Summarizer({ addXP }: any) {
  ...
  const handleGenerate = async () => {
    ...
    setTyped(false);
    try {
      const res = await callAI(`Here are my notes:\n\n${input}`, systemPrompts[mode as keyof typeof systemPrompts]);
      setResult(res);
      addXP(50);
    } catch {
...
// ─── AI HOMEWORK HELPER ───────────────────────────────────────────────────────
function HomeworkHelper({ addXP }: any) {
